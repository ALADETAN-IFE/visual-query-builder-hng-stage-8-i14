...
function highlightSQL(code: string): React.ReactNode[] {
  ...
  return tokens.map((tok, i) => {
    ...
    if (/^(!=|=|>|<)$/.test(tok))
      return (
        <span key={i} className="token-operator">
          {tok}
        </span>
      );
    if (/^--.*/.test(tok))
      return (
        <span key={i} className="token-comment">
          {tok}
        </span>
      );
...
function highlightMongo(code: string): React.ReactNode[] {
  ...
  return lines.flatMap((line, li) => {
    ...
    if (/^\s*\/\//.test(line)) {
      parts.push(
        <span key={`${li}-comment`} className="token-comment">
          {line}
        </span>,
      );
    } else {
      ...
      if (keyMatch) {
        ...
        parts.push(
          ...
          <span
            key={`${li}-key`}
            className={isOperator ? "token-keyword" : "token-field"}
          >{`"${key}"`}</span>,
          <span key={`${li}-colon`} className="token-punctuation">
            {colon}
          </span>,
  ...
  return lines.flatMap((line, li) => {
    ...
    const toks = main
      ...
      .map((tok, i) => {
        ...
        if (/^[{}():,]$/.test(tok))
          return (
            <span key={`${li}-${i}`} className="token-punctuation">
              {tok}
            </span>
          );
    ...
    if (comment)
      result.push(
        <span key={`${li}-cm`} className="token-comment">
          {comment}
        </span>,
      );
  ...
  return (
    <div className="panel p-5 flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between text-[0.6875rem] text-text-tertiary">
        <span>
          {queryText.split("\n").length} lines · {queryText.length} chars
        </span>
